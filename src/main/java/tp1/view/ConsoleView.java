...
public class ConsoleView extends GameView {
	...
	private static final String ROW_BORDER = repeat(CELL_BORDER, GameStatus.DIM_X);
	...
	public ConsoleView(GameStatus status) {
		super(status);
	...
	private String getInfo() {
		...
		/* @formatter:off */
		buffer
		.append(Messages.REMAINING_TIME.formatted(status.remainingTime())).append(NEW_LINE)
		.append(Messages.POINTS.formatted(status.points())).append(NEW_LINE)
		.append(Messages.NUM_LIVES.formatted(status.numLives())).append(NEW_LINE);
		/* @formatter:on */
	...
	public String toString() {
		...
		// Paint game board
		str.append(colLine(GameStatus.DIM_X));
		str.append(LATERAL_TAB);
		str.append(UPPER_ROW_BORDER);

		for (int row = 0; row < GameStatus.DIM_Y; row++) {
			str.append(MyStringUtils.right( getRowName(row) , LATERAL_TAB_SIZE) );
			str.append(VERTICAL_DELIMITER);

			for (int col = 0; col < GameStatus.DIM_X; col++) {
				str.append(consoleCell(status.positionToString(col, row)));
				//str.append(VERTICAL_DELIMITER);
			}
			str.append(VERTICAL_DELIMITER);
	...
	private String endMessage() {
		...
		if(status.playerWins()) sb.append(Messages.MARIO_WINS);
		else if (status.playerLoses()) sb.append(Messages.GAME_OVER);
